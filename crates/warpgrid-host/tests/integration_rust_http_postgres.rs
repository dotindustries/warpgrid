...
//!
//! End-to-end test that exercises a Wasm guest component simulating a Rust
//! axum handler with sqlx through WarpGrid's engine with DNS and database
//! proxy shims.
//!
//! The guest component (built from `tests/fixtures/rust-http-postgres-guest/`)
//! imports the WarpGrid DNS and database-proxy WIT shims and exports test
//! functions that exercise the same shim chain a Rust axum handler compiled
//! to wasm32-wasip2 would use: DNS resolve → database proxy connect →
//! Postgres wire protocol send/recv → close.
//!
//! When Domain 2 (wasi-libc patches) is complete, the Rust guest will be
//! replaced by the actual axum handler compiled with the patched sysroot.
//!
//! Test stack:
//! ```text
...
fn build_guest_component() -> &'static [u8] {
    COMPONENT_BYTES.get_or_init(|| {
        ...
        let guest_dir = root.join("tests/fixtures/rust-http-postgres-guest");
...
async fn test_invalid_db_host_returns_error() {
    ...
    // the nonexistent host, which the real axum handler would map to HTTP 503.
...
/// Validates that axum's TCP connections route through the shim.
#[tokio::test(flavor = "multi_thread")]
...
/// Exercises the complete shim chain that a Rust axum handler would use.
///
