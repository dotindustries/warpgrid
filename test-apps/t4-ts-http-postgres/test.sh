#!/usr/bin/env bash
#
# test.sh — Integration tests for the T4 TypeScript HTTP + Postgres handler.
#
# Tests:
...
#   6. process.env.APP_NAME accessible in X-App-Name response header
...
if [ "${HTTP_CODE}" = "200" ]; then
    # Check that we got an array with 5 users
...
# ── Test 6: X-App-Name header from process.env ──────────────────────
...
if echo "${HEADERS}" | grep -iq "x-app-name"; then
    ...
    # process.env may not be available in standalone mode — acceptable skip
    skip "X-App-Name header not present (process.env may not be available in standalone mode)"
fi
