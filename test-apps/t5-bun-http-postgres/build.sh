#!/usr/bin/env bash
#
# build.sh — Componentize the T5 Bun HTTP handler into a WASI Wasm component.
#
# Modes:
#   (default)     Build the full handler with warpgrid:shim imports
#   --standalone  Build the standalone handler (in-memory data, no shim deps)
#
# This follows the `warp pack --lang bun` pipeline:
#   1. (In full mode: bun build → single-file bundle; standalone mode skips this)
#   2. jco componentize → WASI HTTP Wasm component
#   3. wasm-tools validation
#
# Prerequisites:
...
if [ "${MODE}" = "standalone" ]; then
    ...
    log "Building standalone Bun handler (in-memory data)..."
else
    HANDLER="${SCRIPT_DIR}/src/handler.js"
    WORLD_NAME="handler"
    log "Building full Bun handler (warpgrid:shim database proxy)..."
fi
