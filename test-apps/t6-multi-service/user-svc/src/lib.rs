...
fn pg_simple_query(query: &str) -> Vec<u8> {
    ...
    let length = (4 + query_bytes.len() + 1) as u32; // length includes self + query + null
...
fn do_pg_handshake(handle: u64, user: &str, database: &str) -> Result<Vec<u8>, String> {
    ...
    // Read handshake response (AuthenticationOk + ReadyForQuery = 15 bytes).
...
impl Guest for Component {
    /// Handles user CRUD operations via Postgres.
    ///
    /// POST /users — inserts a user, returns 201
    /// GET /users — selects all users, returns 200
    fn handle_request(
        method: String,
        _path: String,
        body: String,
        ...
        // Verify handshake succeeded (first byte 'R' = AuthenticationOk).
        ...
        let result = if method == "POST" {
            let query = format!(
                "INSERT INTO test_users (name) VALUES ('{body}') RETURNING id, name"
            );
            let msg = pg_simple_query(&query);
            database_proxy::send(handle, &msg)?;
            let resp = database_proxy::recv(handle, 4096)?;
            // Mock server echoes the query — return the echoed bytes as proof of delivery.
            let resp_str = String::from_utf8(resp.into())
                .unwrap_or_else(|_| String::from("<binary>"));
            Ok((201, format!("user_created:{resp_str}"), String::new()))
        } else {
            let msg = pg_simple_query("SELECT * FROM test_users");
            database_proxy::send(handle, &msg)?;
            let resp = database_proxy::recv(handle, 4096)?;
            let resp_str = String::from_utf8(resp.into())
                .unwrap_or_else(|_| String::from("<binary>"));
            Ok((200, format!("users:{resp_str}"), String::new()))
        };
