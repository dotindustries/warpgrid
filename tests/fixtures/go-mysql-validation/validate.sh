#!/usr/bin/env bash
#
# validate.sh — Validate go-sql-driver/mysql compilation with TinyGo wasip2.
#
# US-308: Database driver compatibility — MySQL and Redis
#
# This script runs the full MySQL driver validation sequence:
...
if [ "${TINYGO_AVAILABLE}" = "true" ]; then
    ...
    if COMPILE_OUTPUT=$("${TINYGO_BIN}" build \
        -target=wasip2 \
        -o "${DIST_DIR}/mysql-validation.wasm" \
        "${SCRIPT_DIR}/main.go" 2>&1); then

        COMPILE_STATUS="pass"
        WASM_SIZE="$(wc -c < "${DIST_DIR}/mysql-validation.wasm" | tr -d ' ')"
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║  go-sql-driver/mysql Validation (US-308)     ║"
echo "╠══════════════════════════════════════════════╣"
echo "║  Go tests:           PASS                    ║"
