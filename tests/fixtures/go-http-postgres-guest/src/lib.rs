//! Guest Wasm component simulating a Go net/http + pgx handler for T3.
//!
//! This component imports DNS and database-proxy WIT shims and exports
//! test functions that exercise the same shim chain a patched TinyGo
//! handler would use: DNS resolve → database proxy connect → Postgres
//! wire protocol send/recv → close.
//!
//! When Domain 3 (TinyGo WASI overlay) is complete, this Rust guest
//! will be replaced by the actual Go handler compiled with warp-tinygo.
...
impl Guest for Component {
    ...
    /// Validates that net.Dial TCP data routes through the database proxy.
